...
module PrometheusFindsStats
  ...
  def self.find_matches_for(file, missing, missing_photos)
    ...
    File.open("data/#{file}.csv").each do |l|
      ...
      if file != 'samples'
        missing_photos << find.registration_number unless Find.check_image(find.registration_number)
      end
      unless @items.select{|item| item.formatted_locus_code == find.locus && item.field_number.to_i == find.field_number.to_i && item.pail_number.to_i == find.pail_number.to_i }.present?
        missing << "#{find.registration_number}"
      end
    end
  end
