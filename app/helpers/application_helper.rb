module ApplicationHelper
  ...
  def input_for(form_definition_hash, value, description_type)
    ...
    when "picker"
      if form_definition_hash['values'].is_a? Hash
        select_tag "locus[#{description_type}][#{form_definition_hash['key']}]", options_for_select(@descriptions['lookups'][form_definition_hash['values']['from']], value), include_blank: true, class: "form-control"
      else
  ...
  def input_with_full_param_name(form_definition_hash, value, full_param_name)
    ...
    when "picker"
      if form_definition_hash['values'].is_a? Hash
        select_tag "#{full_param_name}", options_for_select(@descriptions['lookups'][form_definition_hash['values']['from']], value), include_blank: true, class: "form-control"
      else
        select_tag "#{full_param_name}", options_for_select(form_definition_hash['values'], value), include_blank: true, class: "form-control"
      end
    when "text_field"
